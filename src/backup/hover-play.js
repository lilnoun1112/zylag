  ...
  pairs.forEach((video) => {
    ...
    function captureFrame(timestamp) {
      ...
      if (!lastCaptureTime || timestamp - lastCaptureTime >= FRAME_INTERVAL) {
        if (forwardFrameCounter % FORWARD_SKIP === 0) {
          ...
          if (frameCache.length < MAX_FRAMES) frameCache.push(frame);
        }
    ...
    function playReverse(index) {
      if (!reversing || index < 0 || !frameCache[index]) {
        cleanupTimeout = setTimeout(() => { frameCache = []; }, 1000);
        return;
      }
      ctx.putImageData(frameCache[index], 0, 0);
      reverseFrame = requestAnimationFrame(() => playReverse(index - REVERSE_SKIP));
    }
    ...
    container.addEventListener('mouseenter', () => {
      if (activeInstance && activeInstance !== stopCurrent) activeInstance();
    ...
    container.addEventListener('mouseleave', () => {
      ...
      if (activeInstance === stopCurrent) activeInstance = null;
    });
